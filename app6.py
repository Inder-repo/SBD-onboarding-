...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS notifications (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id INTEGER NOT NULL, request_id INTEGER,
        title TEXT NOT NULL, body TEXT NOT NULL,
        is_read INTEGER DEFAULT 0, notif_type TEXT DEFAULT 'info',
        created_at TEXT NOT NULL);

    CREATE TABLE IF NOT EXISTS sla_config (
        phase TEXT PRIMARY KEY, days_allowed INTEGER NOT NULL,
        updated_at TEXT NOT NULL);

    CREATE TABLE IF NOT EXISTS risk_exceptions (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        request_id INTEGER NOT NULL, description TEXT NOT NULL,
        risk_owner TEXT NOT NULL, review_date TEXT NOT NULL,
        accepted_by INTEGER NOT NULL, status TEXT DEFAULT 'open',
        created_at TEXT NOT NULL);

    CREATE TABLE IF NOT EXISTS user_cover (
        user_id INTEGER PRIMARY KEY, cover_user_id INTEGER NOT NULL,
...
def _dedup_questions(c):
    """Remove duplicate questions, keeping only the lowest id per (text, parent_question_id)."""
    dupes = c.execute("""
        SELECT MIN(id) as keep_id, text, COALESCE(parent_question_id, -1) as p
        FROM questions
        GROUP BY text, COALESCE(parent_question_id, -1)
        HAVING COUNT(*) > 1
    """).fetchall()
    if not dupes:
        return
    # Collect all IDs to delete (duplicates with id > keep_id)
    ids_to_delete = []
    for row in dupes:
        extras = c.execute(
            "SELECT id FROM questions WHERE text=? AND COALESCE(parent_question_id,-1)=? AND id != ?",
            (row['text'], row['p'], row['keep_id'])
        ).fetchall()
        ids_to_delete.extend(e['id'] for e in extras)
    if ids_to_delete:
        # Temporarily disable FK checks so we can clean up in order
        c.execute("PRAGMA foreign_keys=OFF")
        # Remove answers referencing duplicate question ids first
        ph = ','.join('?' * len(ids_to_delete))
        c.execute(f"DELETE FROM request_answers WHERE question_id IN ({ph})", ids_to_delete)
        # Remove children whose parent will be deleted
        c.execute(f"DELETE FROM questions WHERE parent_question_id IN ({ph})", ids_to_delete)
        # Remove the duplicate root questions themselves
        c.execute(f"DELETE FROM questions WHERE id IN ({ph})", ids_to_delete)
        # Reassign any remaining children pointing to dead parents
        c.execute("""DELETE FROM questions WHERE parent_question_id NOT IN
                     (SELECT id FROM questions WHERE parent_question_id IS NULL)""")
        c.execute("PRAGMA foreign_keys=ON")
        c.commit()
...
def _seed(c):
    ...
    # Only seed questions if none exist yet — prevents duplicates on restart
    existing_q_count = c.execute("SELECT COUNT(*) FROM questions").fetchone()[0]
    if existing_q_count == 0:
        ...
        for (order, text, desc, hint, opts, wts, max_s, cat, parent_order, trigger) in raw_qs:
            if parent_order is None:
                ...
                order_to_id[order] = c.execute("SELECT last_insert_rowid()").fetchone()[0]
    ...
    for k,v in [("threshold_no_sbd","20"),("threshold_stage1","40"),
                ...
                ("smtp_from","sbd@company.com"),("smtp_enabled","0"),
                ("jira_url",""),("jira_project","SBD"),("jira_enabled","0"),
                ("confluence_url",""),("confluence_enabled","0"),
                ("slack_webhook",""),("slack_enabled","0")]:
        try: c.execute("INSERT OR IGNORE INTO sbd_config VALUES (?,?,?)",(k,v,now))
        except: pass
    for phase,days in [("pending_review",3),("architect_assigned",10),
                       ("engineer_assigned",7),("assurance_assigned",7),("pending_signoff",5)]:
        try: c.execute("INSERT OR IGNORE INTO sla_config VALUES (?,?,?)",(phase,days,now))
        except: pass
    c.commit()
...
def create_request(name, desc, owner, ptype, golive, uid, priority='normal', due_date=''):
    ...
    rid = _exe_id("""INSERT INTO requests
        (ref_number,project_name,project_description,business_owner,project_type,
         go_live_date,created_by,status,priority,due_date,created_at,updated_at)
        VALUES (?,?,?,?,?,?,?,'draft',?,?,?,?)""",(ref,name,desc,owner,ptype,golive,uid,priority,due_date or None,now,now))
...
def get_sla_config():
    return {r['phase']:r['days_allowed'] for r in _qry("SELECT * FROM sla_config")}

def days_in_phase(req):
    last = _qry("SELECT created_at FROM status_history WHERE request_id=? ORDER BY created_at DESC LIMIT 1",(req['id'],),one=True)
    if not last: return 0
    try: return max(0,(datetime.now()-datetime.fromisoformat(last['created_at'])).days)
    except: return 0

def sla_status(req):
    days = days_in_phase(req); sla = get_sla_config()
    allowed = sla.get(req.get('status',''),99)
    if allowed==99 or req.get('status') in ('draft','signoff_received','no_sbd_needed'): return None,0,0
    return days>allowed, days, allowed

def check_sla_breaches():
    active = _qry("SELECT * FROM requests WHERE status IN ('pending_review','architect_assigned','engineer_assigned','assurance_assigned','pending_signoff') AND sla_breach=0")
    sla = get_sla_config()
    for r in active:
        if days_in_phase(r) > sla.get(r['status'],99):
            _exe("UPDATE requests SET sla_breach=1 WHERE id=?",(r['id'],))

def notify(uid,title,body,ntype='info',rid=None):
    _exe("INSERT INTO notifications (user_id,request_id,title,body,notif_type,created_at) VALUES (?,?,?,?,?,?)",
         (uid,rid,title,body,ntype,datetime.now().isoformat()))

def get_notifs(uid,unread_only=False):
    sql = "SELECT n.*,r.ref_number FROM notifications n LEFT JOIN requests r ON n.request_id=r.id WHERE n.user_id=?"
    if unread_only: sql += " AND n.is_read=0"
    return _qry(sql+" ORDER BY n.created_at DESC LIMIT 60",(uid,))

def mark_read(uid): _exe("UPDATE notifications SET is_read=1 WHERE user_id=?",(uid,))
def mark_one_read(nid): _exe("UPDATE notifications SET is_read=1 WHERE id=?",(nid,))

def get_cover(uid):
    today = datetime.now().strftime('%Y-%m-%d')
    return _qry("""SELECT uc.*,u.name as cover_name,u.username as cover_uname
        FROM user_cover uc JOIN users u ON uc.cover_user_id=u.id
        WHERE uc.user_id=? AND uc.start_date<=? AND uc.end_date>=?
        ORDER BY uc.created_at DESC LIMIT 1""",(uid,today,today),one=True)

def set_cover(uid,cid,start,end,by):
    _exe("INSERT OR REPLACE INTO user_cover (user_id,cover_user_id,start_date,end_date,created_by,created_at) VALUES (?,?,?,?,?,?)",
         (uid,cid,start,end,by,datetime.now().isoformat()))

def clear_cover(uid): _exe("DELETE FROM user_cover WHERE user_id=?",(uid,))

def get_workload():
    arch  = _qry("SELECT u.id,u.name,COUNT(r.id) as open FROM users u LEFT JOIN requests r ON r.architect_id=u.id AND r.status='architect_assigned' WHERE u.role='security_architect' AND u.is_active=1 GROUP BY u.id")
    eng   = _qry("SELECT u.id,u.name,COUNT(r.id) as open FROM users u LEFT JOIN requests r ON r.engineer_id=u.id AND r.status='engineer_assigned' WHERE u.role='security_engineer' AND u.is_active=1 GROUP BY u.id")
    assur = _qry("SELECT u.id,u.name,COUNT(r.id) as open FROM users u LEFT JOIN requests r ON r.assurance_id=u.id AND r.status='assurance_assigned' WHERE u.role='assurance' AND u.is_active=1 GROUP BY u.id")
    return arch, eng, assur

def add_exception(rid,desc,owner,review,by):
    _exe("INSERT INTO risk_exceptions (request_id,description,risk_owner,review_date,accepted_by,created_at) VALUES (?,?,?,?,?,?)",
         (rid,desc,owner,review,by,datetime.now().isoformat()))

def get_exceptions(rid=None):
    base = "SELECT re.*,u.name as acc_name FROM risk_exceptions re JOIN users u ON re.accepted_by=u.id"
    if rid: return _qry(base+" WHERE re.request_id=? ORDER BY re.created_at DESC",(rid,))
    return _qry("""SELECT re.*,u.name as acc_name,r.ref_number,r.project_name
        FROM risk_exceptions re JOIN users u ON re.accepted_by=u.id
        JOIN requests r ON re.request_id=r.id ORDER BY re.created_at DESC""")

def close_exception(eid): _exe("UPDATE risk_exceptions SET status='closed' WHERE id=?",(eid,))

def get_webhooks(): return _qry("SELECT * FROM webhooks WHERE is_active=1")

def fire_webhook(event,payload):
    import urllib.request as ur
    hooks = _qry("SELECT * FROM webhooks WHERE is_active=1 AND (events='all' OR events LIKE ?)",(f'%{event}%',))
    for h in hooks:
        try:
            d=json.dumps({**payload,'event':event,'ts':datetime.now().isoformat()}).encode()
            ur.urlopen(ur.Request(h['url'],d,{'Content-Type':'application/json','X-SbD-Event':event}),timeout=4)
        except: pass

def get_report_data():
    total = _qry("SELECT COUNT(*) as c FROM requests",one=True)['c']
    by_outcome = _qry("SELECT sbd_outcome,COUNT(*) as c FROM requests WHERE sbd_outcome IS NOT NULL GROUP BY sbd_outcome")
    by_status  = _qry("SELECT status,COUNT(*) as c FROM requests GROUP BY status")
    by_prio    = _qry("SELECT priority,COUNT(*) as c FROM requests GROUP BY priority")
    monthly    = _qry("SELECT strftime('%Y-%m',created_at) as m,COUNT(*) as c FROM requests GROUP BY m ORDER BY m DESC LIMIT 6")
    avg_days   = _qry("""SELECT sbd_outcome,ROUND(AVG(julianday(COALESCE(signoff_received_at,datetime('now')))-julianday(submitted_at)),1) as avg
        FROM requests WHERE submitted_at IS NOT NULL GROUP BY sbd_outcome""")
    overdue    = _qry("""SELECT r.*,u.name as owner_name FROM requests r JOIN users u ON r.created_by=u.id
        WHERE r.sla_breach=1 AND r.status NOT IN ('draft','signoff_received')""")
    exceptions = get_exceptions()
    return dict(total=total,by_outcome=by_outcome,by_status=by_status,by_prio=by_prio,
                monthly=monthly,avg_days=avg_days,overdue=overdue,exceptions=exceptions)

PRIORITY_CFG = {
    'low':      ('Low',      '#6B7280','#F9FAFB'),
    'normal':   ('Normal',   '#3B82F6','#EFF6FF'),
    'high':     ('High',     '#F59E0B','#FFFBEB'),
    'critical': ('Critical', '#EF4444','#FEF2F2'),
}
...
.badge{display:inline-flex;align-items:center;gap:.35rem;padding:.3rem .85rem;border-radius:999px;font-size:.78rem;font-weight:700;text-transform:uppercase;letter-spacing:.04em;white-space:nowrap;}
...
.pipeline{display:flex;align-items:center;margin:1rem 0 .5rem;overflow-x:auto;padding-bottom:.5rem;}
.pipe-step{display:flex;flex-direction:column;align-items:center;flex:1;min-width:72px;}
.pipe-node{width:44px;height:44px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1.15rem;z-index:1;flex-shrink:0;}
.pn-todo{background:var(--surface3);border:2.5px solid var(--border2);color:var(--text4);}
.pn-done{background:var(--success);border:2.5px solid var(--success);color:white;}
.pn-now{background:var(--accent);border:2.5px solid var(--accent);color:white;box-shadow:0 0 0 6px rgba(59,130,246,.18);}
.pipe-lbl{font-size:.72rem;text-align:center;margin-top:.5rem;color:var(--text2);font-weight:600;text-transform:uppercase;letter-spacing:.03em;max-width:76px;line-height:1.35;}
.pl{flex:1;height:3px;margin-bottom:1.35rem;}
...
def pipeline_html(current):
    ...
    for i,(key,label,icon) in enumerate(PIPELINE):
        ...
        # Done: bold checkmark; Current: show the phase emoji; Todo: empty
        if i < ci:   ni = '✓'
        elif i==ci:  ni = icon
        else:        ni = ''
        lbl_style = ' style="color:var(--accent);font-weight:700;"' if i==ci else (' style="color:var(--success);"' if i<ci else '')
        if i>0: html += f'<div class="pl {"pl-done" if i<=ci else "pl-todo"}"></div>'
        html += f'<div class="pipe-step"><div class="pipe-node {nc}">{ni}</div><div class="pipe-lbl"{lbl_style}>{label}</div></div>'
    return html+'</div>'
...
def render_sidebar(user):
    role = user['role']
    # Sidebar visibility toggle — persists in session state
    if 'sb_open' not in st.session_state: st.session_state.sb_open = True

    # CSS: show/hide sidebar via JavaScript class toggle workaround
    if not st.session_state.sb_open:
        st.markdown("""<style>
        [data-testid="stSidebar"]{display:none!important;}
        [data-testid="stSidebarCollapsedControl"]{display:none!important;}
        .main .block-container{padding-left:1.75rem!important;max-width:100%!important;}
        </style>""", unsafe_allow_html=True)
    else:
        st.markdown("""<style>
        [data-testid="stSidebarCollapsedControl"]{display:none!important;}
    ...
    with st.sidebar:
        ...
        with cols_sb[1]:
            ...
            if st.button("✕", key="sb_close", help="Hide sidebar", use_container_width=True):
        ...
        if role in ['sbd_manager','admin']:
            ...
            nav += [("📥","Review Queue","pending_review"),("👥","Assign Resources","assign_resources"),
                    ...
                    ("📈","Programme Report","report"),("⚠️","Risk Register","risk_register"),
                    ("🔗","Integrations","integrations"),("📋","Team Capacity","capacity")]
        if role in ['security_architect','security_engineer','assurance','sbd_manager']:
            st.markdown('<div class="sb-sec">My Account</div>', unsafe_allow_html=True)
            nav.append(("🔄","My Cover","my_cover"))
        if role=='admin':
        ...
        st.markdown("---")
        _u2=st.session_state.user['id'] if st.session_state.get('user') else None
        if _u2:
            _nr=len(get_notifs(_u2,unread_only=True))
            _lb=f"🔔  Notifications{(' ('+str(_nr)+')') if _nr else ''}"
            if st.button(_lb,key="nav_notif",use_container_width=True,type="primary" if st.session_state.get('page')=="notifications" else "secondary"):
                st.session_state.page="notifications"; st.rerun()
        if st.button("🚪  Sign Out", use_container_width=True): logout(); st.rerun()

    # When sidebar is hidden, show a floating reopen button in main content
    if not st.session_state.sb_open:
        st.markdown("""<div style="position:fixed;top:14px;left:14px;z-index:9999;">
        </div>""", unsafe_allow_html=True)
        if st.button("☰  Menu", key="sb_open_btn"):
...
def page_request_detail(user):
    ...
    role=user['role']
    is_mgr=role in ['admin','sbd_manager']          # can VIEW management tabs
    is_sbd_mgr=role=='sbd_manager'                   # ONLY sbd_manager assigns & signs off
    req=get_req(rid)
    ...
    with tabs[0]: _rd_overview(req,user,has_write,is_locked,is_mgr,is_sbd_mgr,assigned,is_submitter)
...
def _rd_overview(req,user,has_write,is_locked,is_mgr,is_sbd_mgr,assigned,is_submitter):
    c1,c2=st.columns([3,2])
    with c1:
    ...
    # ── Actions — visible to assigned team + sbd_manager (not after final signoff) ─
    if req['status'] != 'signoff_received':
        _rd_actions(req,user,is_sbd_mgr,assigned)

def _rd_actions(req,user,is_sbd_mgr,assigned):
    status=req['status']; rid=req['id']; uid=user['id']

    is_architect = assigned and req.get('architect_id')==uid
    is_engineer  = assigned and req.get('engineer_id')==uid
    is_assurance = assigned and req.get('assurance_id')==uid

    if not (is_sbd_mgr or is_architect or is_engineer or is_assurance):
        return

    st.markdown("---")
    st.markdown('<div style="font-weight:700;font-size:.9rem;margin-bottom:.875rem;">Actions</div>', unsafe_allow_html=True)

    # ── SbD Manager: review gating, phase hand-offs, final sign-off ──────────
    if is_sbd_mgr:
        ...
        if status=='pending_review':
            if btn("✅ Confirm & Assess","act_confirm"):
                update_status(rid,'awaiting_assignment',uid,'Review confirmed'); st.rerun()
            with cols[ci[0]%5]:
                st.markdown('<div style="font-size:.72rem;color:#64748B;margin-bottom:.2rem;">Return for revision:</div>', unsafe_allow_html=True)
            ci[0]+=1
            with cols[ci[0]%5]:
                reason=st.text_input("Reason",key=f"ri_{rid}",label_visibility="collapsed",placeholder="Reason...")
            ci[0]+=1
            if btn("↩ Return to Applicant","act_return",primary=False):
                if reason.strip():
                    return_to_draft(rid,uid,reason.strip())
                    add_comment(rid,uid,f"Returned for revision: {reason.strip()}",internal=False)
                    st.success("Returned to applicant."); st.rerun()
                else: st.error("Please provide a reason.")
        if status=='architect_completed':
            if btn("➡ Move to Engineer","act_toeng"):
                update_status(rid,'engineer_assigned',uid,'Assigned to engineering phase'); st.rerun()
        if status=='engineer_completed':
            if btn("➡ Move to Assurance","act_toassur"):
                update_status(rid,'assurance_assigned',uid,'Assigned to assurance phase'); st.rerun()
        if status=='assurance_completed':
            if btn("➡ Move to Sign-off","act_tosignoff"):
                update_status(rid,'pending_signoff',uid,'Moved to sign-off queue'); st.rerun()
        if status=='pending_signoff':
            if btn("🎉 Record Sign-off","act_signoff"):
                update_status(rid,'signoff_received',uid,'Sign-off received',{'signoff_by':uid}); st.rerun()

    # ── Architect: completion button + doc/notes (architect sees this) ────────
    if is_architect or is_sbd_mgr:
        if status in ['architect_assigned','architect_completed','engineer_assigned',
                      'engineer_completed','assurance_assigned','assurance_completed',
                      'pending_signoff','signoff_received']:
            with st.expander("🏗 Architecture", expanded=(status=='architect_assigned')):
                if status=='architect_assigned':
                    st.markdown('<div class="alert as" style="margin-bottom:.75rem;">Mark complete when your architecture review is finished. The SbD Manager will then assign the engineering phase.</div>', unsafe_allow_html=True)
                    if st.button("✅ Mark Architecture Complete", key="act_archdone", type="primary", use_container_width=True):
                        update_status(rid,'architect_completed',uid,'Architecture review completed')
                        notify(req.get('created_by'),'Architecture review complete',
                               f"Architecture phase done for {req['ref_number']}",'info',rid)
                        st.success("Architecture phase marked complete."); st.rerun()
                ...
                if st.button("💾 Save",key="save_arch",type="secondary"):
                    c2=_conn(); c2.execute("UPDATE requests SET architect_url=?,architect_notes=? WHERE id=?",(au,an,rid)); c2.commit(); c2.close()
                    st.success("Saved."); st.rerun()

    # ── Engineer: completion button + notes (engineer sees this) ─────────────
    if is_engineer or is_sbd_mgr:
        if status in ['engineer_assigned','engineer_completed','assurance_assigned',
                      'assurance_completed','pending_signoff']:
            with st.expander("⚙ Engineering", expanded=(status=='engineer_assigned')):
                if status=='engineer_assigned':
                    st.markdown('<div class="alert as" style="margin-bottom:.75rem;">Mark complete when your engineering review is finished. The SbD Manager will then assign the assurance phase.</div>', unsafe_allow_html=True)
                    if st.button("✅ Mark Engineering Complete", key="act_engdone", type="primary", use_container_width=True):
                        update_status(rid,'engineer_completed',uid,'Engineering work completed')
                        notify(req.get('created_by'),'Engineering review complete',
                               f"Engineering phase done for {req['ref_number']}",'info',rid)
                        st.success("Engineering phase marked complete."); st.rerun()
                en=st.text_area("Notes",value=req.get('engineer_notes',''),key="eng_notes",height=100)
                if st.button("💾 Save",key="save_eng",type="secondary"):
                    c2=_conn(); c2.execute("UPDATE requests SET engineer_notes=? WHERE id=?",(en,rid)); c2.commit(); c2.close()
                    st.success("Saved."); st.rerun()

    # ── Assurance: completion button + notes (assurance sees this) ───────────
    if is_assurance or is_sbd_mgr:
        if status in ['assurance_assigned','assurance_completed','pending_signoff']:
            with st.expander("🔍 Assurance", expanded=(status=='assurance_assigned')):
                if status=='assurance_assigned':
                    st.markdown('<div class="alert as" style="margin-bottom:.75rem;">Mark complete when your assurance review is finished. The SbD Manager will then move the request to sign-off.</div>', unsafe_allow_html=True)
                    if st.button("✅ Mark Assurance Complete", key="act_assurdone", type="primary", use_container_width=True):
                        update_status(rid,'assurance_completed',uid,'Assurance review completed')
                        notify(req.get('created_by'),'Assurance review complete',
                               f"Assurance phase done for {req['ref_number']}",'info',rid)
                        st.success("Assurance phase marked complete."); st.rerun()
                asn=st.text_area("Notes",value=req.get('assurance_notes',''),key="assur_notes",height=100)
                if st.button("💾 Save",key="save_assur",type="secondary"):
...
def page_pending_review(user):
    ...
    _po={'critical':0,'high':1,'normal':2,'low':3}
    reqs=sorted(reqs_by_status(['pending_review']),key=lambda r:(_po.get(r.get('priority','normal'),2),r.get('created_at','')))
    if not reqs:
...
def page_assign_resources(user):
    ...
    _po2={'critical':0,'high':1,'normal':2,'low':3}
    reqs=sorted(reqs_by_status(['awaiting_assignment','architect_assigned','architect_completed',
                          'engineer_assigned','engineer_completed','assurance_assigned','assurance_completed']),key=lambda r:(_po2.get(r.get('priority','normal'),2),r.get('created_at','')))
...
# ══════════════════════════════════════════════════════════════════════════════
# RISK EXCEPTIONS TAB (used inside request detail)
# ══════════════════════════════════════════════════════════════════════════════

def _rd_risk_tab(req, user, is_mgr):
    rid = req['id']
    excs = get_exceptions(rid)
    st.markdown('<div class="alert aw" style="margin-bottom:1rem;">Risk exceptions are formal acknowledgements of identified security risks that cannot be immediately remediated. Each exception requires a named owner and a scheduled review date.</div>', unsafe_allow_html=True)
    if not excs:
        st.markdown('<div style="text-align:center;padding:2rem;color:#94A3B8;font-size:.85rem;">No risk exceptions recorded for this request.</div>', unsafe_allow_html=True)
    else:
        for ex in excs:
            sc = "#10B981" if ex['status'] == 'closed' else "#F59E0B"
            bg = "#F0FDF4" if ex['status'] == 'closed' else "#FFFBEB"
            bd = "#BBF7D0" if ex['status'] == 'closed' else "#FDE68A"
            st.markdown(f'''<div style="padding:.875rem 1rem;border:1px solid {bd};border-left:3px solid {sc};
                border-radius:8px;margin-bottom:.75rem;background:{bg};">
              <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:.5rem;">
                <div style="flex:1;">
                  <div style="font-weight:600;font-size:.88rem;color:#1E293B;">{ex["description"]}</div>
                  <div style="font-size:.75rem;color:#64748B;margin-top:.35rem;">
                    Owner: <strong>{ex["risk_owner"]}</strong> &nbsp;·&nbsp;
                    Review by: <strong>{ex["review_date"]}</strong> &nbsp;·&nbsp;
                    Accepted by: {ex["acc_name"]}
                  </div>
                </div>
                <span class="badge" style="background:{bg};color:{sc};border:1px solid {bd};white-space:nowrap;">{ex["status"].upper()}</span>
              </div>
            </div>''', unsafe_allow_html=True)
            if is_mgr and ex['status'] == 'open':
                if st.button("✅ Close Exception", key=f"closex_{ex['id']}"):
                    close_exception(ex['id']); st.rerun()

    if is_mgr:
        st.markdown("---")
        st.markdown('<div style="font-weight:600;font-size:.9rem;margin-bottom:.75rem;">➕ Log New Risk Exception</div>', unsafe_allow_html=True)
        with st.form(f"rex_form_{rid}"):
            rx_desc = st.text_area("Risk description *", height=90,
                placeholder="Describe the risk, what control is missing, and why it is being accepted at this time...")
            rc1, rc2 = st.columns(2)
            with rc1: rx_owner = st.text_input("Risk Owner *", placeholder="Name or role responsible for reviewing")
            with rc2: rx_date  = st.text_input("Review Date *", placeholder="dd/mm/yyyy")
            if st.form_submit_button("Log Exception", type="primary"):
                if rx_desc.strip() and rx_owner.strip() and rx_date.strip():
                    add_exception(rid, rx_desc.strip(), rx_owner.strip(), rx_date.strip(), user['id'])
                    add_comment(rid, user['id'], f"Risk exception logged: {rx_desc[:80]}...", internal=True)
                    st.success("Exception logged and noted in internal comments."); st.rerun()
                else: st.error("All fields are required.")


# ══════════════════════════════════════════════════════════════════════════════
# PAGE: PROGRAMME REPORT (sbd_manager + admin only)
# ══════════════════════════════════════════════════════════════════════════════

def page_report(user):
    if user['role'] not in ['sbd_manager', 'admin']:
        st.error("🚫 Access restricted to SbD Manager and Administrators."); return

    st.markdown('<div class="pg-hdr"><div class="pg-title">📈 Programme Report</div><div class="pg-sub">Programme-wide metrics, SLA performance and trend analysis</div></div>', unsafe_allow_html=True)

    d = get_report_data()
    sla_cfg = get_sla_config()

    # ── Top KPI row ───────────────────────────────────────────────────────────
    outcome_map = {r['sbd_outcome']: r['c'] for r in d['by_outcome']}
    status_map  = {r['status']: r['c'] for r in d['by_status']}
    overdue_count = len(d['overdue'])

    c1,c2,c3,c4,c5,c6 = st.columns(6)
    for col, (icon, label, val, color) in zip(
        [c1,c2,c3,c4,c5,c6], [
            ("📋", "Total Requests", d['total'], "#3B82F6"),
            ("⏳", "In Progress", sum(status_map.get(s,0) for s in ['architect_assigned','engineer_assigned','assurance_assigned']), "#F97316"),
            ("🎉", "Completed", status_map.get('signoff_received', 0), "#10B981"),
            ("🔴", "Overdue (SLA breach)", overdue_count, "#EF4444"),
            ("⚠️", "Risk Exceptions", len(d['exceptions']), "#F59E0B"),
            ("🔶", "Full SbD Required", outcome_map.get('full_sbd', 0), "#8B5CF6"),
        ]):
        with col:
            st.markdown(f'<div class="stat-card" style="border-top:3px solid {color};"><div style="font-size:1.4rem;">{icon}</div><div class="stat-val" style="color:{color};">{val}</div><div class="stat-lbl">{label}</div></div>', unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)

    # ── Row 2: Outcomes + Monthly Trend ──────────────────────────────────────
    col_l, col_r = st.columns(2)

    with col_l:
        st.markdown('<div style="font-weight:700;font-size:.95rem;margin-bottom:.75rem;">SbD Outcome Breakdown</div>', unsafe_allow_html=True)
        for outcome, cfg in OUTCOME_CFG.items():
            cnt = outcome_map.get(outcome, 0)
            pct = round(cnt / d['total'] * 100, 1) if d['total'] else 0
            color = cfg[1]
            st.markdown(f'''<div style="display:flex;align-items:center;gap:.75rem;margin-bottom:.625rem;">
              <div style="width:110px;font-size:.8rem;font-weight:600;color:{color};">{cfg[0]}</div>
              <div style="flex:1;background:#F1F5F9;border-radius:999px;height:10px;overflow:hidden;">
                <div style="background:{color};height:10px;border-radius:999px;width:{pct}%;"></div>
              </div>
              <div style="width:50px;text-align:right;font-size:.8rem;font-weight:700;color:{color};">{cnt}</div>
              <div style="width:38px;text-align:right;font-size:.72rem;color:#94A3B8;">{pct}%</div>
            </div>''', unsafe_allow_html=True)

    with col_r:
        st.markdown('<div style="font-weight:700;font-size:.95rem;margin-bottom:.75rem;">Monthly Submission Volume</div>', unsafe_allow_html=True)
        monthly = list(reversed(d['monthly']))
        if monthly:
            max_m = max((r['c'] for r in monthly), default=1)
            for row in monthly:
                w = round(row['c'] / max_m * 100, 1) if max_m else 0
                st.markdown(f'''<div style="display:flex;align-items:center;gap:.75rem;margin-bottom:.5rem;">
                  <div style="width:65px;font-size:.78rem;color:#64748B;font-weight:600;">{row["m"]}</div>
                  <div style="flex:1;background:#F1F5F9;border-radius:999px;height:10px;">
                    <div style="background:#3B82F6;height:10px;border-radius:999px;width:{w}%;"></div>
                  </div>
                  <div style="width:28px;text-align:right;font-size:.8rem;font-weight:700;color:#3B82F6;">{row["c"]}</div>
                </div>''', unsafe_allow_html=True)
        else:
    ...
    # ── Row 3: Avg completion time + Priority breakdown ───────────────────────
    col_a, col_b = st.columns(2)

    with col_a:
        st.markdown('<div style="font-weight:700;font-size:.95rem;margin-bottom:.75rem;">Average Days to Complete (by Outcome)</div>', unsafe_allow_html=True)
        if d['avg_days']:
            for row in d['avg_days']:
                cfg = OUTCOME_CFG.get(row['sbd_outcome'])
                if not cfg: continue
                days = row['avg'] or 0
                color = cfg[1]
                st.markdown(f'''<div style="display:flex;align-items:center;gap:.75rem;margin-bottom:.625rem;">
                  <div style="width:110px;font-size:.8rem;font-weight:600;color:{color};">{cfg[0]}</div>
                  <div style="flex:1;background:#F1F5F9;border-radius:999px;height:10px;">
                    <div style="background:{color};height:10px;border-radius:999px;width:{min(100,days/30*100):.0f}%;"></div>
                  </div>
                  <div style="width:55px;text-align:right;font-size:.8rem;font-weight:700;color:{color};">{days:.1f}d</div>
                </div>''', unsafe_allow_html=True)
        else:
            st.info("Not enough completed requests to calculate averages yet.")

    with col_b:
        st.markdown('<div style="font-weight:700;font-size:.95rem;margin-bottom:.75rem;">Requests by Priority</div>', unsafe_allow_html=True)
        prio_map = {r['priority']: r['c'] for r in d['by_prio']}
        for p, (l, color, bg) in PRIORITY_CFG.items():
            cnt = prio_map.get(p, 0)
            pct = round(cnt / d['total'] * 100, 1) if d['total'] else 0
            st.markdown(f'''<div style="display:flex;align-items:center;gap:.75rem;margin-bottom:.625rem;">
              <div style="width:70px;font-size:.8rem;font-weight:600;color:{color};">{l}</div>
              <div style="flex:1;background:#F1F5F9;border-radius:999px;height:10px;">
                <div style="background:{color};height:10px;border-radius:999px;width:{pct}%;"></div>
              </div>
              <div style="width:30px;text-align:right;font-size:.8rem;font-weight:700;color:{color};">{cnt}</div>
            </div>''', unsafe_allow_html=True)

    # ── SLA Thresholds config ─────────────────────────────────────────────────
    st.markdown("<br>", unsafe_allow_html=True)
    with st.expander("⚙️ Configure SLA Thresholds (days per phase)"):
        phases = [("pending_review","Initial Review"),("architect_assigned","Architecture"),
                  ("engineer_assigned","Engineering"),("assurance_assigned","Assurance"),
                  ("pending_signoff","Sign-Off")]
        scols = st.columns(5)
        new_vals = {}
        for col, (phase, label) in zip(scols, phases):
            with col:
                new_vals[phase] = st.number_input(label, min_value=1, max_value=60,
                    value=sla_cfg.get(phase, 7), key=f"sla_{phase}")
        if st.button("Save SLA Thresholds", type="primary"):
            c2 = _conn()
            now2 = datetime.now().isoformat()
            for phase, val in new_vals.items():
                c2.execute("INSERT OR REPLACE INTO sla_config VALUES (?,?,?)", (phase, val, now2))
            c2.commit(); c2.close()
            st.success("SLA thresholds updated."); st.rerun()

    # ── Overdue requests table ────────────────────────────────────────────────
    if d['overdue']:
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown('<div style="font-weight:700;font-size:.95rem;margin-bottom:.75rem;color:#EF4444;">🔴 Overdue Requests</div>', unsafe_allow_html=True)
        for req in d['overdue']:
            breached, days, allowed = sla_status(req)
            over = days - allowed
            _,color,_,_ = STATUS_CFG.get(req['status'],('','#6B7280','','?'))
            st.markdown(f'''<div class="card card-sm" style="border-left:3px solid #EF4444;margin-bottom:.5rem;">
              <div style="display:flex;align-items:center;gap:.75rem;justify-content:space-between;">
                <div>
                  <div style="display:flex;gap:.5rem;align-items:center;flex-wrap:wrap;">
                    <span class="ref">{req["ref_number"]}</span>{sbadge(req["status"])}{pbadge(req.get("priority","normal"))}
                  </div>
                  <div style="font-weight:600;font-size:.88rem;margin-top:.2rem;">{req["project_name"]}</div>
                  <div style="font-size:.75rem;color:#94A3B8;">Owner: {req["owner_name"]}</div>
                </div>
                <div style="text-align:right;white-space:nowrap;">
                  <div style="font-size:1.1rem;font-weight:800;color:#EF4444;">{over}d overdue</div>
                  <div style="font-size:.72rem;color:#94A3B8;">{days}d / {allowed}d allowed</div>
                </div>
              </div>
            </div>''', unsafe_allow_html=True)
            if st.button("Open →", key=f"rpt_open_{req['id']}"):
                st.session_state.selected_req = req['id']; st.session_state.page = "request_detail"; st.rerun()


# ══════════════════════════════════════════════════════════════════════════════
# PAGE: RISK REGISTER (sbd_manager + admin only)
# ══════════════════════════════════════════════════════════════════════════════

def page_risk_register(user):
    if user['role'] not in ['sbd_manager', 'admin']:
        st.error("🚫 Access restricted to SbD Manager and Administrators."); return

    st.markdown('<div class="pg-hdr"><div class="pg-title">⚠️ Risk Register</div><div class="pg-sub">All open and closed risk exceptions across the programme</div></div>', unsafe_allow_html=True)

    filt = st.selectbox("Filter by status", ["All", "open", "closed"], label_visibility="collapsed")
    excs = get_exceptions()
    if filt != "All": excs = [e for e in excs if e['status'] == filt]

    if not excs:
        st.info("No risk exceptions match the current filter.")
        return

    st.markdown(f'<div style="font-size:.78rem;color:#64748B;margin-bottom:.75rem;"><strong>{len(excs)}</strong> exception(s)</div>', unsafe_allow_html=True)

    for ex in excs:
        sc = "#10B981" if ex['status'] == 'closed' else "#F59E0B"
        bg = "#F0FDF4" if ex['status'] == 'closed' else "#FFFBEB"
        bd = "#BBF7D0" if ex['status'] == 'closed' else "#FDE68A"
        ref_info = f'<span class="ref">{ex.get("ref_number","—")}</span> {ex.get("project_name","")}'
        ...
        with c1:
            st.markdown(f'''<div style="padding:.875rem 1rem;border:1px solid {bd};border-left:3px solid {sc};
              ...
              <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                <div style="flex:1;">
                  <div style="margin-bottom:.3rem;">{ref_info}</div>
                  <div style="font-weight:600;font-size:.88rem;">{ex["description"]}</div>
                  <div style="font-size:.75rem;color:#64748B;margin-top:.3rem;">
                    Owner: <strong>{ex["risk_owner"]}</strong> &nbsp;·&nbsp; Review by: <strong>{ex["review_date"]}</strong>
                    &nbsp;·&nbsp; Accepted by: {ex["acc_name"]} &nbsp;·&nbsp; Logged: {fdates(ex["created_at"])}
                  </div>
                </div>
                <span class="badge" style="background:{bg};color:{sc};border:1px solid {bd};margin-left:.5rem;">{ex["status"].upper()}</span>
              </div>
            </div>''', unsafe_allow_html=True)
        with c2:
            st.markdown("<div style='padding-top:.75rem;'>", unsafe_allow_html=True)
            if st.button("View Request", key=f"rr_view_{ex['id']}"):
                st.session_state.selected_req = ex['request_id']
                st.session_state.page = "request_detail"; st.rerun()
            if ex['status'] == 'open':
                if st.button("Close", key=f"rr_close_{ex['id']}", type="secondary"):
                    close_exception(ex['id']); st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)


# ══════════════════════════════════════════════════════════════════════════════
# PAGE: TEAM CAPACITY (sbd_manager + admin only)
# ══════════════════════════════════════════════════════════════════════════════

def page_capacity(user):
    if user['role'] not in ['sbd_manager', 'admin']:
        st.error("🚫 Access restricted to SbD Manager and Administrators."); return

    st.markdown('<div class="pg-hdr"><div class="pg-title">📋 Team Capacity</div><div class="pg-sub">Current workload per team member and active cover assignments</div></div>', unsafe_allow_html=True)

    arch_list, eng_list, assur_list = get_workload()

    def workload_section(title, icon, members, max_warn=3):
        st.markdown(f'<div style="font-weight:700;font-size:.95rem;margin-bottom:.75rem;">{icon} {title}</div>', unsafe_allow_html=True)
        if not members:
            st.markdown(f'<div style="color:#94A3B8;font-size:.85rem;">No {title.lower()} found.</div>', unsafe_allow_html=True)
            return
        for m in members:
            cnt = m.get('open', 0)
            color = "#EF4444" if cnt >= max_warn else ("#F59E0B" if cnt >= max_warn - 1 else "#10B981")
            bar_w = min(100, cnt / max(max_warn, 1) * 100)
            cover = get_cover(m['id'])
            cover_tag = f'<span style="font-size:.7rem;background:#EFF6FF;color:#3B82F6;padding:.15rem .4rem;border-radius:4px;margin-left:.4rem;">Cover: {cover["cover_name"]}</span>' if cover else ''
            st.markdown(f'''<div style="display:flex;align-items:center;gap:1rem;padding:.75rem 1rem;
                background:#F8FAFC;border:1px solid #E2E8F0;border-radius:8px;margin-bottom:.5rem;">
              <div style="width:32px;height:32px;background:linear-gradient(135deg,#3B82F6,#8B5CF6);
                   border-radius:50%;display:flex;align-items:center;justify-content:center;
                   color:white;font-weight:700;font-size:.8rem;flex-shrink:0;">{m["name"][0]}</div>
              <div style="flex:1;min-width:0;">
                <div style="font-weight:600;font-size:.88rem;">{m["name"]}{cover_tag}</div>
                <div style="display:flex;align-items:center;gap:.5rem;margin-top:.25rem;">
                  <div style="flex:1;background:#E2E8F0;border-radius:999px;height:6px;">
                    <div style="background:{color};height:6px;border-radius:999px;width:{bar_w:.0f}%;"></div>
                  </div>
                  <div style="font-size:.78rem;font-weight:700;color:{color};white-space:nowrap;">{cnt} active</div>
                </div>
              </div>
            </div>''', unsafe_allow_html=True)

    ca, cb, cc = st.columns(3)
    with ca: workload_section("Security Architects", "🏗", arch_list, max_warn=4)
    with cb: workload_section("Security Engineers",  "⚙️", eng_list,  max_warn=4)
    with cc: workload_section("Assurance Analysts",  "🔍", assur_list,max_warn=4)

    # Active cover assignments
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<div style="font-weight:700;font-size:.95rem;margin-bottom:.75rem;">🔄 Active Cover Assignments</div>', unsafe_allow_html=True)
    today = datetime.now().strftime('%Y-%m-%d')
    covers = _qry("""SELECT uc.*,u.name as user_name,u.role as user_role,cv.name as cover_name
        FROM user_cover uc JOIN users u ON uc.user_id=u.id JOIN users cv ON uc.cover_user_id=cv.id
        WHERE uc.start_date<=? AND uc.end_date>=? ORDER BY uc.start_date""",(today,today))
    if not covers:
        st.info("No active cover assignments today.")
    else:
        for c in covers:
            st.markdown(f'''<div style="padding:.75rem 1rem;background:#F0FDF4;border:1px solid #BBF7D0;
                border-radius:8px;margin-bottom:.5rem;display:flex;align-items:center;gap:1rem;">
              <div style="flex:1;font-size:.87rem;">
                <strong>{c["user_name"]}</strong> ({ROLE_LABELS.get(c["user_role"],c["user_role"])})
                is covered by <strong>{c["cover_name"]}</strong>
              </div>
              <div style="font-size:.75rem;color:#64748B;">{c["start_date"]} → {c["end_date"]}</div>
            </div>''', unsafe_allow_html=True)


# ══════════════════════════════════════════════════════════════════════════════
# PAGE: MY COVER (delegation for SbD team members)
# ══════════════════════════════════════════════════════════════════════════════

def page_my_cover(user):
    if user['role'] not in ['security_architect', 'security_engineer', 'assurance', 'sbd_manager']:
        st.error("🚫 Not available for your role."); return

    st.markdown('<div class="pg-hdr"><div class="pg-title">🔄 My Cover</div><div class="pg-sub">Set a colleague to cover your assignments while you are away</div></div>', unsafe_allow_html=True)

    current = get_cover(user['id'])
    if current:
        st.markdown(f'''<div style="padding:1rem 1.25rem;background:#ECFDF5;border:1px solid #6EE7B7;
            border-radius:10px;margin-bottom:1.25rem;">
          <div style="font-weight:700;color:#065F46;margin-bottom:.35rem;">✅ Active Cover Assignment</div>
          <div style="font-size:.88rem;color:#047857;">
            <strong>{current["cover_name"]}</strong> (@{current["cover_uname"]}) is covering for you
            from <strong>{current["start_date"]}</strong> to <strong>{current["end_date"]}</strong>.
          </div>
        </div>''', unsafe_allow_html=True)
        if st.button("🗑 Remove Cover Assignment", type="secondary"):
            clear_cover(user['id'])
            st.success("Cover assignment removed."); st.rerun()
    else:
        st.markdown('<div class="alert ai" style="margin-bottom:1rem;">No active cover assignment. Set one below before going on leave so your requests don\'t stall.</div>', unsafe_allow_html=True)

    st.markdown('<div style="font-weight:600;font-size:.9rem;margin-bottom:.75rem;">Set New Cover</div>', unsafe_allow_html=True)
    same_role_users = [u for u in all_users(user['role']) if u['id'] != user['id']]
    if not same_role_users:
        st.warning("No other users with your role found to assign as cover.")
        return

    with st.form("cover_form"):
        cover_opts = {u['id']: f"{u['name']} (@{u['username']})" for u in same_role_users}
        sel_cover = st.selectbox("Cover colleague", list(cover_opts.keys()), format_func=lambda x: cover_opts[x])
        c1, c2 = st.columns(2)
        with c1: start = st.text_input("Start Date (YYYY-MM-DD)", placeholder=datetime.now().strftime('%Y-%m-%d'))
        with c2: end   = st.text_input("End Date (YYYY-MM-DD)", placeholder=(datetime.now().strftime('%Y-%m-%d')))
        if st.form_submit_button("Set Cover", type="primary"):
            if start and end and start <= end:
                set_cover(user['id'], sel_cover, start, end, user['id'])
                notify(sel_cover, "You have been assigned as cover",
                       f"{user['name']} has set you as their cover from {start} to {end}.", 'info')
                st.success("Cover assignment saved."); st.rerun()
            else: st.error("Valid start and end dates required (start must be ≤ end).")
...
def page_notifications(user):
    st.markdown('<div class="pg-hdr"><div class="pg-title">🔔 Notifications</div><div class="pg-sub">Your recent alerts and assignments</div></div>', unsafe_allow_html=True)

    notifs = get_notifs(user['id'])
    unread = [n for n in notifs if not n['is_read']]

    c1, c2 = st.columns([6, 1])
    with c1:
        st.markdown(f'<div style="font-size:.82rem;color:#64748B;">{len(unread)} unread · {len(notifs)} total</div>', unsafe_allow_html=True)
    with c2:
        if unread and st.button("Mark all read", use_container_width=True):
            mark_read(user['id']); st.rerun()

    if not notifs:
        st.markdown('<div style="text-align:center;padding:3rem;color:#94A3B8;font-size:.9rem;">No notifications yet. You\'ll see assignment alerts and submission updates here.</div>', unsafe_allow_html=True)
        return

    NTYPE_CFG = {
        'assignment': ('#EFF6FF', '#3B82F6', '📌'),
        'info':       ('#F8FAFC', '#64748B', 'ℹ️'),
        'warning':    ('#FFFBEB', '#F59E0B', '⚠️'),
        'urgent':     ('#FEF2F2', '#EF4444', '🔴'),
    }

    for n in notifs:
        bg, color, icon = NTYPE_CFG.get(n.get('notif_type','info'), NTYPE_CFG['info'])
        unread_dot = f'<span style="width:8px;height:8px;background:#3B82F6;border-radius:50%;display:inline-block;margin-right:.4rem;"></span>' if not n['is_read'] else ''
        ref_tag = f'<span class="ref" style="margin-left:.5rem;">{n["ref_number"]}</span>' if n.get('ref_number') else ''

        c_main, c_act = st.columns([8, 1])
        with c_main:
            st.markdown(f'''<div style="padding:.75rem 1rem;background:{bg};border:1px solid {color}22;
                border-left:3px solid {color};border-radius:8px;margin-bottom:.375rem;
                opacity:{"1" if not n["is_read"] else ".65"};">
              <div style="display:flex;align-items:flex-start;gap:.5rem;">
                <span style="font-size:1rem;flex-shrink:0;">{icon}</span>
                <div style="flex:1;">
                  <div style="display:flex;align-items:center;gap:.25rem;flex-wrap:wrap;">
                    {unread_dot}<strong style="font-size:.87rem;">{n["title"]}</strong>{ref_tag}
                  </div>
                  <div style="font-size:.82rem;color:#475569;margin-top:.15rem;">{n["body"]}</div>
                  <div style="font-size:.7rem;color:#94A3B8;margin-top:.2rem;">{fdate(n["created_at"])}</div>
                </div>
              </div>
            </div>''', unsafe_allow_html=True)
        with c_act:
            if n.get('request_id'):
                if st.button("Open", key=f"notif_open_{n['id']}"):
                    mark_one_read(n['id'])
                    st.session_state.selected_req = n['request_id']
                    st.session_state.page = "request_detail"; st.rerun()


# ══════════════════════════════════════════════════════════════════════════════
# PAGE: INTEGRATIONS (sbd_manager + admin)
# ══════════════════════════════════════════════════════════════════════════════

def page_integrations(user):
    if user['role'] not in ['sbd_manager', 'admin']:
        st.error("🚫 Access restricted to SbD Manager and Administrators."); return

    st.markdown('<div class="pg-hdr"><div class="pg-title">🔗 Integrations</div><div class="pg-sub">Configure external system connections and webhook endpoints</div></div>', unsafe_allow_html=True)

    cfg = get_cfg()
    tabs = st.tabs(["📧 Email / SMTP", "🎫 Jira", "📚 Confluence", "💬 Slack", "🪝 Webhooks"])

    # ── Email ─────────────────────────────────────────────────────────────────
    with tabs[0]:
        st.markdown('<div class="alert ai" style="margin-bottom:1rem;">When enabled, SbD Portal will send assignment and submission notifications via SMTP. Passwords are stored in the config table — use a dedicated service account.</div>', unsafe_allow_html=True)
        with st.form("smtp_form"):
            enabled = st.toggle("Enable email notifications", value=cfg.get('smtp_enabled','0')=='1')
            c1,c2 = st.columns(2)
            with c1: host = st.text_input("SMTP Host", value=cfg.get('smtp_host',''), placeholder="smtp.company.com")
            with c2: port = st.text_input("SMTP Port", value=cfg.get('smtp_port','587'))
            c3,c4 = st.columns(2)
            with c3: uname = st.text_input("SMTP Username", value=cfg.get('smtp_user',''))
            with c4: pwd   = st.text_input("SMTP Password", type="password", placeholder="Leave blank to keep existing")
            frm = st.text_input("From Address", value=cfg.get('smtp_from','sbd@company.com'))
            if st.form_submit_button("Save Email Settings", type="primary"):
                set_cfg('smtp_enabled', '1' if enabled else '0')
                set_cfg('smtp_host', host); set_cfg('smtp_port', port)
                set_cfg('smtp_user', uname); set_cfg('smtp_from', frm)
                if pwd: set_cfg('smtp_password', pwd)
                st.success("Email settings saved.")

    # ── Jira ──────────────────────────────────────────────────────────────────
    with tabs[1]:
        st.markdown('<div class="alert ai" style="margin-bottom:1rem;">When enabled, a Jira issue will be created automatically when a request is assigned to the architect phase. Requires a Jira API token.</div>', unsafe_allow_html=True)
        with st.form("jira_form"):
            jenabled = st.toggle("Enable Jira integration", value=cfg.get('jira_enabled','0')=='1')
            c1,c2 = st.columns(2)
            with c1: jurl = st.text_input("Jira Base URL", value=cfg.get('jira_url',''), placeholder="https://company.atlassian.net")
            with c2: jproj = st.text_input("Project Key", value=cfg.get('jira_project','SBD'))
            c3,c4 = st.columns(2)
            with c3: juser = st.text_input("API Username / Email", value=cfg.get('jira_user',''))
            with c4: jtok = st.text_input("API Token", type="password", placeholder="Leave blank to keep existing")
            if st.form_submit_button("Save Jira Settings", type="primary"):
                set_cfg('jira_enabled', '1' if jenabled else '0')
                set_cfg('jira_url', jurl); set_cfg('jira_project', jproj)
                set_cfg('jira_user', juser)
                if jtok: set_cfg('jira_token', jtok)
                st.success("Jira settings saved.")

    # ── Confluence ────────────────────────────────────────────────────────────
    with tabs[2]:
        st.markdown('<div class="alert ai" style="margin-bottom:1rem;">When enabled, a Confluence page will be created upon sign-off to serve as the permanent security assessment record for the project.</div>', unsafe_allow_html=True)
        with st.form("conf_form"):
            cenabled = st.toggle("Enable Confluence integration", value=cfg.get('confluence_enabled','0')=='1')
            c1,c2 = st.columns(2)
            with c1: curl = st.text_input("Confluence Base URL", value=cfg.get('confluence_url',''), placeholder="https://company.atlassian.net/wiki")
            with c2: cspace = st.text_input("Space Key", value=cfg.get('confluence_space','SBD'))
            if st.form_submit_button("Save Confluence Settings", type="primary"):
                set_cfg('confluence_enabled', '1' if cenabled else '0')
                set_cfg('confluence_url', curl); set_cfg('confluence_space', cspace)
                st.success("Confluence settings saved.")

    # ── Slack ─────────────────────────────────────────────────────────────────
    with tabs[3]:
        st.markdown('<div class="alert ai" style="margin-bottom:1rem;">Post notifications to a Slack channel via an Incoming Webhook URL. Alerts are sent on submission, assignment, and sign-off events.</div>', unsafe_allow_html=True)
        with st.form("slack_form"):
            senabled = st.toggle("Enable Slack notifications", value=cfg.get('slack_enabled','0')=='1')
            swh = st.text_input("Slack Incoming Webhook URL", value=cfg.get('slack_webhook',''), placeholder="https://hooks.slack.com/services/...")
            schan = st.text_input("Channel (optional override)", value=cfg.get('slack_channel',''), placeholder="#sbd-notifications")
            if st.form_submit_button("Save Slack Settings", type="primary"):
                set_cfg('slack_enabled', '1' if senabled else '0')
                set_cfg('slack_webhook', swh); set_cfg('slack_channel', schan)
                st.success("Slack settings saved.")
                if senabled and swh:
                    try:
                        import urllib.request as ur2
                        test_data = json.dumps({"text": "✅ SbD Portal Slack integration test — connection successful."}).encode()
                        ur2.urlopen(ur2.Request(swh, test_data, {'Content-Type':'application/json'}), timeout=5)
                        st.success("Test message sent to Slack successfully.")
                    except Exception as e:
                        st.error(f"Slack test failed: {e}")

    # ── Webhooks ──────────────────────────────────────────────────────────────
    with tabs[4]:
        st.markdown('<div class="alert ai" style="margin-bottom:1rem;">Outbound webhooks fire a JSON POST to your URL on specified events. Events: <code>request.submitted</code>, <code>request.architect_assigned</code>, <code>request.signoff_received</code>, or <code>all</code>.</div>', unsafe_allow_html=True)
        hooks = _qry("SELECT * FROM webhooks ORDER BY created_at DESC")
        if hooks:
            for h in hooks:
                c1,c2,c3 = st.columns([4,2,1])
                with c1: st.markdown(f'<div style="font-size:.85rem;font-weight:600;">{h["name"]}</div><div style="font-size:.75rem;color:#64748B;">{h["url"]}</div>', unsafe_allow_html=True)
                with c2: st.markdown(f'<code style="font-size:.72rem;">{h["events"]}</code>', unsafe_allow_html=True)
                with c3:
                    if st.button("Delete", key=f"delwh_{h['id']}", type="secondary"):
                        _exe("DELETE FROM webhooks WHERE id=?", (h['id'],)); st.rerun()
        else:
            st.info("No webhooks configured yet.")
        st.markdown("---")
        with st.form("wh_form"):
            wname = st.text_input("Webhook Name", placeholder="e.g. Jira Alert")
            wurl  = st.text_input("URL", placeholder="https://...")
            wevt  = st.text_input("Events (comma-separated or 'all')", value="all")
            if st.form_submit_button("Add Webhook", type="primary"):
                if wname and wurl:
                    _exe("INSERT INTO webhooks (name,url,events,is_active,created_at) VALUES (?,?,?,1,?)",
                         (wname, wurl, wevt, datetime.now().isoformat()))
                    st.success("Webhook added."); st.rerun()
